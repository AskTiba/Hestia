# Progress Tracking: ShopMaster

## Phase 1: Foundation (MVP)
- [ ] Initialize Expo Project (React Native + TypeScript) <!-- id: 0 -->
- [ ] Configure NativeWind for styling <!-- id: 1 -->
- [x] Setup NestJS + Prisma Backend Foundation <!-- id: 2 -->
- [ ] Implement Basic Routing (Expo Router) <!-- id: 3 -->
- [x] Authentication & RBAC <!-- id: 4 -->
- [ ] Inventory CRUD <!-- id: 5 -->

## Phase 2: Location & Staff Mode
- [ ] Indoor Mapping UI <!-- id: 6 -->
- [ ] Product Location Assignment <!-- id: 7 -->
- [ ] Staff Mode & Price Lookup <!-- id: 8 -->

## Phase 3: Finance & Analytics
- [ ] POS Lite (Transaction Logging) <!-- id: 9 -->
- [ ] Financial Dashboard <!-- id: 10 -->
- [ ] Low Stock Alerts <!-- id: 11 -->

## Phase 4: Offline & Polish
- [ ] Offline Support (SQLite + TanStack Query) <!-- id: 12 -->
- [ ] Export to CSV/PDF <!-- id: 13 -->
- [ ] Dark Mode & UI Polish <!-- id: 14 -->
