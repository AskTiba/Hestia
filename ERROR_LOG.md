...
| 2026-05-13 | `nest generate : Cannot find module './core'` | Corrupted `node_modules` or `ajv` package in CLI path. | **Resolved**: Performed a clean `rm -rf node_modules` and fresh `bun install`. |
...
| 2026-05-13 | `PrismaClientInitializationError: PrismaClient needs to be constructed with options` | Prisma 7 runtime requires explicit options when URL is missing from schema. | **Resolved**: Installed `@prisma/adapter-pg` and updated `PrismaService` to use a database adapter. |
| 2026-05-13 | `EADDRINUSE: address already in use :::3000 / :::3001` | The default NestJS port (3000) or an updated port (3001) was blocked by an existing process. | **Resolved**: Updated `main.ts` to listen on 3001 (and set in `.env`) to avoid conflicts with frontend frameworks. Managed background processes correctly. |
